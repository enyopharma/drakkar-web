...
final class PopulatePublicationResponder
{
     ...
     * @return mixed
     */
    public function write(OutputInterface $output, PopulatePublicationResult $result)
    {
        return $result->match([
            PopulatePublicationResult::SUCCESS => function (int $pmid) use ($output) {
                return $output->writeln(
                    sprintf('<info>Metadata of publication with [\'pmid\' => %s] successfully updated.</info>', $pmid)
                );
            },
            PopulatePublicationResult::ALREADY_POPULATED => function (int $pmid) use ($output) {
                return $output->writeln(
                    sprintf('<info>Metadata of publication with [\'pmid\' => %s] already populated</info>', $pmid)
                );
            },
            PopulatePublicationResult::NOT_FOUND => function (int $pmid) use ($output) {
                return $output->writeln(
                    sprintf('<error>No publication with [\'pmid\' => %s]</error>', $pmid)
                );
            },
            PopulatePublicationResult::PARSING_ERROR => function (int $pmid, string $message) use ($output) {
                return $output->writeln(
