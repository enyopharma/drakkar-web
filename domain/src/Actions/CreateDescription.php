...
SQL;
    ...
    public function __invoke(array $input): DomainPayloadInterface
    {
        ...
        try {
            ...
            return new InputNotValid([$e->getMessage()]);
        }
        ...
        if (! $method = $select_method_sth->fetch()) {
            return new DomainConflict('Method not found');
        }
        ...
        if (! $protein1 = $select_protein_sth->fetch()) {
            return new DomainConflict('Interactor 1 protein not found');
        }
        ...
        if (! $protein2 = $select_protein_sth->fetch()) {
            return new DomainConflict('Interactor 2 protein not found');
        }

        // ensure type of interactor1 is human.
        if ($protein1['type'] != Protein::H) {
            return new DomainConflict('Interactor 1 protein must be a human protein');
        }

        // ensure type of interactor2 is matching the curation run type.
        if ($association['type'] == Run::HH && $protein2['type'] == Protein::V) {
            return new DomainConflict('Interactor 2 protein must be a human protein');
        }

        if ($association['type'] == Run::VH && $protein2['type'] == Protein::H) {
            return new DomainConflict('Interactor 2 protein must be a viral protein');
        }
        ...
        if ($interactor1['stop'] > strlen($sequence['sequence'])) {
            return new DomainConflict('Interactor 1 stop is greater than the protein sequence length');
        }
        ...
        if ($interactor2['stop'] > strlen($sequence['sequence'])) {
            return new DomainConflict('Interactor 2 stop is greater than the protein sequence length');
        }
        ...
        if ($row = $select_interactor_name_sth->fetch()) {
            if ($row['name'] != $interactor1['name']) {
                return new DomainConflict('Interactor 1 name is not valid');
            }
        }
        ...
        if ($row = $select_interactor_name_sth->fetch()) {
            if ($row['name'] != $interactor2['name']) {
                return new DomainConflict('Interactor 2 name is not valid');
            }
        }
        ...
        if ($row = $select_interactor_pos_sth->fetch()) {
            if ($row['start'] != $interactor1['start'] || $row['stop'] != $interactor1['stop']) {
                return new DomainConflict('Interactor 1 coordinates are not valid');
            }
        }
        ...
        if ($row = $select_interactor_pos_sth->fetch()) {
            if ($row['start'] != $interactor2['start'] || $row['stop'] != $interactor2['stop']) {
                return new DomainConflict('Interactor 2 coordinates are not valid');
            }
        }
        ...
        foreach ($interactor1['mapping'] as $alignment) {
            foreach ($alignment['isoforms'] as $isoform) {
                ...
                if (! $sequence = $select_sequence_sth->fetch()) {
                    return new DomainConflict('Interactor 1 mapping is not valid');
                }
        ...
        foreach ($interactor2['mapping'] as $alignment) {
            foreach ($alignment['isoforms'] as $isoform) {
                ...
                if (! $sequence = $select_sequence_sth->fetch()) {
                    return new DomainConflict('Interactor 2 mapping is not valid');
                }
